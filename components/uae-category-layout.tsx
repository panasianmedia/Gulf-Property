...
interface UAECategoryLayoutProps {
  cityTitle: string // "Abu Dhabi", "Dubai", "Ras Al Khaimah", "Sharjah"
  ...
  spotlightArticles: Article[] // 4 City Spotlight Articles
  section5Title?: string
}

// Subcategories list under UAE
const UAE_SUB_CATEGORIES = [
  "Abu Dhabi",
  "Dubai",
  "Ras Al Khaimah",
  "Sharjah",
...
function UAECategoryContent({
  ...
  opinionArticles,
  spotlightArticles,
  section5Title = "City Spotlight",
}: UAECategoryLayoutProps) {
  return (
      ...
      <header className="border-b border-border bg-background">
        <div className="mx-auto max-w-7xl px-4 pt-6 pb-4">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <nav className="flex flex-wrap gap-2 pt-2 md:pt-0" aria-label="UAE subcategories">
              {UAE_SUB_CATEGORIES.map((sub) => {
                const subSlug = sub.toLowerCase().replace(/\s+/g, "-")
                const isActive = sub.toLowerCase() === cityTitle.toLowerCase()
                return (
                  <Link
                    key={sub}
                    href={`/uae/${subSlug}`}
...
export function UAECategoryLayout(props: UAECategoryLayoutProps) {
  return (
    <Suspense fallback={<div className="p-8 text-center text-muted-foreground">Loading UAE coverage...</div>}>
      <UAECategoryContent {...props} />
    </Suspense>
  )
}